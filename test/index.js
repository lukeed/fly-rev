...
const fly = new Fly({
	...
	tasks: {
		a: function * (o) {
			const t = o.val;
			yield this.source(glob).rev().target(tmp);
			t.true('rev' in fly, 'attach `rev()` plugin to fly');
			t.false(yield this.$.find(`${tmp}/a.js`), 'rename the file');
			t.true(yield this.$.find(`${tmp}/a-${hash1}.js`), 'generate content-based hash');
			t.true(yield this.$.find(`${tmp}/b.svg`), 'ignore matching `opts.ignore` file types');
			t.true(yield this.$.find(`${tmp}/sub/c-${hash2}.js`), 'rename sub-level files');
			yield this.clear(tmp);
		},
		b: function * (o) {
			const t = o.val;
			yield this.source(glob).rev().revManifest({dest: tmp}).target(tmp);
		...
		c: function * (o) {
			const t = o.val;
			yield this.source(glob).rev().revManifest({trim: 'test'}).target(`${tmp}/sub`);
